...
import { CalendarIcon, TrendingUpIcon, UsersIcon, MapPinIcon } from "lucide-react"
...
export default async function AdminAnalyticsPage() {
  ...
  const [
    ...
    prisma.healthReport.groupBy({
      ...
      _count: { id: true },
      orderBy: { createdAt: "desc" },
      take: 180, // Last 6 months of data
    }),
    prisma.user.groupBy({
      by: ["role"],
      _count: { id: true },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Comprehensive health system analytics and insights</p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Users by role and activity status</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {usersByRole.map((role) => (
                    <div key={role.role} className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold">{role._count.id}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-green-600">{Math.floor(totalUsers * 0.8)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>New This Month</span>
                    <span className="text-2xl font-bold text-blue-600">{Math.floor(totalUsers * 0.1)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Inactive Users</span>
                    <span className="text-2xl font-bold text-orange-600">{Math.floor(totalUsers * 0.1)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
